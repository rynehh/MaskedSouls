...
function updateUI(runtime: IRuntime, gameTime: number, players: any[]) {
    ...
    for (const p of players) {
        ...
        // =========================================================
        // <--- NUEVO: LOGICA DE OPACIDAD (MUERTE)
        // =========================================================
        // Si la vida es 0 o tiene la flag IsDead, bajamos la opacidad a 0.4 (grisaceo)
        // Si está vivo, la opacidad es 1 (totalmente visible)
        const isDead = (player.instVars.HP <= 0) || player.instVars.IsDead;
        const uiOpacity = isDead ? 0.4 : 1; 
        // =========================================================
        ...
        if (myScoreTxt) {
            ...
            myScoreTxt.opacity = uiOpacity; // <--- APLICAMOS OPACIDAD
        }

        // ===============================================
        // B y C. NOMBRE DE MÁSCARA E ÍCONO (UNIFICADOS)
        // ===============================================
        ...
        if (myMaskTxt) {
            ...
            myMaskTxt.opacity = uiOpacity; // <--- APLICAMOS OPACIDAD
            
            // Colores (Tu lógica actual)
            if (currentMask === "Fuego") myMaskTxt.fontColor = [1, 0.2, 0.2];
            else if (currentMask === "Hielo") myMaskTxt.fontColor = [0.2, 0.8, 1];
            else if (currentMask === "Rayo") myMaskTxt.fontColor = [1, 1, 0]; 
        ...
        if (myIconMask) {
            myIconMask.opacity = uiOpacity; // <--- APLICAMOS OPACIDAD

            if (myIconMask.animationName !== currentMask) {
                try {
                    myIconMask.setAnimation(currentMask);
                } catch (e) {
                    console.log("No existe animación para: " + currentMask);
                    myIconMask.setAnimation("Normal"); 
                }
            }
        }
        ...
        if (myHearts.length > 0) {
            ...
            const hp = Number(player.instVars.HP || 200);
            const maxHP = 200;
            ...
            myHearts.forEach((heart, index) => {
                ...
                h.blendMode = "normal"; 
                h.opacity = uiOpacity; // <--- APLICAMOS OPACIDAD A CADA CORAZÓN

                if (hp >= thresholdHigh) { 
                    h.animationFrame = 0; // Lleno
                } else if (hp <= thresholdLow) { 
                    h.animationFrame = 3; // Vacío
                } else { 
